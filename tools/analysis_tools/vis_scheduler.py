...
class FakeMobileNetV3(MobileNetV3):
    def __init__(
        self,
        **kwargs
    ) -> None:
...
def fake_mobilenet_v3_small(num_classes: int = 1000, reduced_tail: bool = False):

    width_multi = 0.1
    bneck_conf = partial(InvertedResidualConfig, width_mult=width_multi)
    adjust_channels = partial(InvertedResidualConfig.adjust_channels, width_mult=width_multi)

    reduce_divider = 2 if reduced_tail else 1

    inverted_residual_setting = [
        bneck_conf(1, 1, 1, 1, False, "RE", 1, 1),  # C1
    ]
    last_channel = adjust_channels(1 // reduce_divider)  # C5

    return FakeMobileNetV3(inverted_residual_setting=inverted_residual_setting,
...
def main():
    ...
    # init logger
    print(f'Param_scheduler : {cfg.param_scheduler}')
