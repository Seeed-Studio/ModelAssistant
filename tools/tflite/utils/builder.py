...
def representative_image_dataset_gen(img_root, img_size, mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]):
    '''
        generate data for quantization with images datasets.
...
def representative_audio_dataset_gen(audio_root, audio_size=[8192]):
    """
        generate data for quantization with audio datasets(only support wav format).
    """

    if not os.path.exists(audio_root):
        raise Exception(f'audio root path: {audio_root} not exists, please check!')
    else:
        for i, fn in enumerate(os.listdir(audio_root)):
            if fn.split(".")[-1].lower() != 'wav':
                continue
            _input, _ = librosa.core.load(os.path.join(audio_root, fn), sr=None, mono=True)
            _input = 0.95 * librosa.util.normalize(_input)[:audio_size[0]]
            _input = _input[:, None]
...
def keras_builder(onnx_model, new_input_nodes: list = None, new_output_nodes: list = None):
    ...
    for inp in model_graph.input:
        # if dynamic input, x.dim_param exists and be None.
        input_shape = [x.dim_value if x.dim_value else None for x in inp.type.tensor_type.shape.dim]
        if input_shape == []:
            continue
...
def tflite_builder(keras_model, weight_quant: bool = False, int8_model: bool = False, data_root: str = None,
                   int8_mean: list or float = None, int8_std: list or float = None):
    ...
    if int8_model:
        input_shape = [keras_model.inputs[0].shape[1], keras_model.inputs[0].shape[2]] \
            if len(keras_model.inputs[0].shape) == 4 else [keras_model.inputs[0].shape[1]]
        if len(input_shape) == 2:
            converter.representative_dataset = lambda: representative_image_dataset_gen(data_root, input_shape)
        else:
            converter.representative_dataset = lambda: representative_audio_dataset_gen(data_root, input_shape)
