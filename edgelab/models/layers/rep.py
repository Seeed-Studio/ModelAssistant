...
from edgelab.registry import FUNCTIONS
...
def fuse_conv_norm(block: Union[nn.Sequential, nn.BatchNorm2d, nn.LayerNorm],
    ...
    if isinstance(block, nn.Sequential):
        conv_weight,conv_bias, norm_mean, norm_var, norm_gamm, norm_beta, norm_eps = \
            block.conv.weight,block.conv.bias, block.norm.running_mean, block.norm.running_var, block.norm.weight, block.norm.bias, block.norm.eps

        std = (norm_var + norm_eps).sqrt()
        t = (norm_gamm / std).reshape(-1, 1, 1, 1)
        return conv_weight * t, norm_beta + (0 if conv_bias is None else
                                             conv_bias -
...
class VanillaBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = ConvNormActivation(in_channels,
                                        out_channels,
                                        1,
                                        bias=True,
                                        norm_layer=norm_config,
                                        activation_layer=None)
        if add_pool:
            self.pool = nn.Identity() if stride == 1 else nn.AdaptiveMaxPool2d(
                (add_pool, add_pool))
        else:
    ...
    def rep(self) -> None:
        ...
        weight = weight2.transpose(1, 3).matmul(
            weight1.squeeze(3).squeeze(2)).transpose(1, 3)
