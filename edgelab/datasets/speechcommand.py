...
class Speechcommand(Dataset):
    ...
    def load_meta_file(self, root, f_meta):
        ...
        with open(filepath) as fileobj:
            ...
            fnames = [
                ...
                for line in fileobj
                if os.path.basename(os.path.dirname(os.path.join(root, line.strip()))) in self.words
            ]
        return fnames
    ...
    def __getitem__(self, index):
        ...
        self.trans = torch.nn.Sequential(torchaudio.transforms.Resample(sampling_rate, 8000, rolloff=0.5))
    ...
    def evaluate(self, results, **kwargs):
        return {
            'loss': torch.mean(torch.tensor([i['loss'] for i in results])).cpu().item(),
