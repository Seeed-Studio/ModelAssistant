...
class ResBlock1dTF(nn.Module):
    def __init__(self, dim, dilation=1, kernel_size=3):
        ...
        self.block_f = nn.Sequential(
            nn.Conv1d(dim, dim, 1, 1, bias=False), nn.BatchNorm1d(dim), nn.LeakyReLU(0.2, True)
        )
...
class SoundNetRaw(nn.Module):
    def __init__(self, nf=2, clip_length=None, factors=[4, 4, 4], out_channel=32):
        super().__init__()
        model = [
        ...
        for _, f in enumerate(factors):
            for i in range(1):
                for j in range(3):
                    model += [ResBlock1dTF(dim=nf, dilation=3**j, kernel_size=7)]
            model += [Down(channels=nf, d=f, k=f * 2 + 1)]
            nf *= 2
