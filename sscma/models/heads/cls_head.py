...
class ClsHead(BaseHead):
    ...
    def loss(self, feats: Tuple[torch.Tensor], data_samples: List[ClsDataSample], **kwargs) -> dict:
    ...
    def _get_loss(self, cls_score: torch.Tensor, data_samples: List[ClsDataSample], **kwargs):
        ...
        if self.cal_acc:
            assert target.ndim == 1, (
                'If you enable batch augmentation ' 'like mixup during training, `cal_acc` is pointless.'
            )
    ...
    def predict(
        self, feats: Tuple[torch.Tensor], data_samples: List[Union[ClsDataSample, None]] = None
    ) -> List[ClsDataSample]:
