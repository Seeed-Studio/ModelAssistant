...
def padding_weights(
    ...
    if isinstance(shape, int):
        ...
        raise TypeError(
            'Wrong shape type, its type should be "int", "tuple", or "list",but got the type of {}'.format(
                type(shape)
            )
        )
...
def fuse_conv_norm(
    ...
    if isinstance(block, nn.Sequential):
        ...
        return (
            conv_weight * t,
            norm_beta
            + ((0 if conv_bias is None else conv_bias) - norm_mean) * norm_gamm / std,
        )
        ...
        w = torch.zeros(
            (in_channels, b, 1, 1), dtype=torch.float32, device=norm_gamm.device
        )
        ...
        raise TypeError(
            "Fusion module type should be Sequential or BatchNorm2d, but got {} type".format(
                type(block)
            )
        )
...
class RepConv1x1(BaseModule):
    def __init__(
        ...
        self.conv3x3 = ConvNormActivation(
            in_channels, out_channels, 3, stride, 1, bias=True, activation_layer=None
        )
        self.conv = nn.ModuleList()

        for i in range(depth):
            layer = ConvNormActivation(
                out_channels, out_channels, 1, 1, 0, bias=True, activation_layer=None
            )
        ...
        self.fuse_conv = nn.Conv2d(
            in_channels, out_channels, 3, padding=1, stride=stride, bias=True
        )
        ...
        if len(self.conv) > 1:
            for idx, layer in enumerate(self.conv[1:]):
                ...
                if self.use_res:
                    ...
                    weight_ += norm_weight
                    bias_ += norm_bias
                    weight = nn.functional.conv2d(
                        weight.transpose(0, 1), weight_
                    ).transpose(0, 1)
...
class Activation(nn.ReLU):
    ...
    def __init__(self, in_channels: int, act_num: int = 3) -> None:
        ...
        self.weight = torch.nn.Parameter(
            torch.randn(in_channels, 1, act_num * 2 + 1, act_num * 2 + 1)
        )
    ...
    def rep(self) -> None:
        ...
        weight, bias = (
            ...
            norm_beta - norm_gamm * norm_mean / std,
        )
...
class VanillaBlock(nn.Module):
    ...
    def __init__(
        ...
        if add_pool:
            self.pool = (
                nn.Identity()
                if stride == 1
                else nn.AdaptiveMaxPool2d((add_pool, add_pool))
            )
        else:
    ...
    def rep(self) -> None:
        ...
        weight = (
            weight2.transpose(1, 3)
            .matmul(weight1.squeeze(3).squeeze(2))
            .transpose(1, 3)
        )
