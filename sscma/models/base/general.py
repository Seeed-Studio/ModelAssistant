...
def get_conv(conv):
    ...
    elif (isinstance(conv, type.__class__) and issubclass(conv, nn.Module)) or hasattr(
        conv, "__call__"
    ):
        pass
    else:
...
def get_norm(norm):
    ...
    elif (isinstance(norm, type.__class__) and issubclass(norm, nn.Module)) or hasattr(
        norm, "__call__"
    ):
        pass
    else:
...
def get_act(act):
    if isinstance(act, dict) and hasattr(nn, act.get("type")):
        ...
        act = MODELS.get(act)
    elif (isinstance(act, type.__class__) and issubclass(act, nn.Module)) or hasattr(
        act, "__call__"
    ):
        pass
    else:
...
class ConvNormActivation(nn.Sequential):
    def __init__(
        ...
        groups: int = 1,
        norm_layer: Union[
            Optional[Callable[..., nn.Module]], Dict, AnyStr
        ] = nn.BatchNorm2d,
        activation_layer: Union[
            Optional[Callable[..., nn.Module]], Dict, AnyStr
        ] = nn.ReLU,
        conv_layer: Union[Optional[Callable[..., nn.Module]], Dict, AnyStr] = None,
        dilation: int = 1,
        inplace: bool = True,
    ...
    return nn.Sequential(
        ...
        (
            ...
            if not act
                ...
                else getattr(nn, act)(inplace=True)
            )
        ),
    )
