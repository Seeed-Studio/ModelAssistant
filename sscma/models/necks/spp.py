...
class SPP(nn.Module):
    def __init__(self, input_channels, output_channels, layers=[1, 2, 3]) -> None:
        ...
        for idx, value in enumerate(layers):
            layer = self._make_layer(output_channels, output_channels, groups=output_channels, number=value)
            setattr(self, f'layer{idx}', layer)

        self.CB = nn.Sequential(
            nn.Conv2d(output_channels * len(layers), output_channels, 1, 1, 0, bias=False),
    ...
    def _make_layer(self, inp, oup, kernel=5, stride=1, padding=2, groups=1, bias=False, number=1):
        layer = []
        for _ in range(number):
            layer.append(CBR(inp, oup, kernel, stride, padding=padding, bias=bias, groups=groups))
