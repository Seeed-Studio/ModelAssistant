...
def _download(url: str, api_key: Optional[str] = None) -> str:
    ...
    if not is_link(url):
        raise ValueError(
            f'Checked "{url}" is not a link, please check the download link and try again.'
        )
    ...
    if api_key is None:
        ...
        if os.path.exists(extrapath):
            ...
            raise ConnectionError(
                "'Connection timed out, please check network connection and retry!'"
            )
        ...
        raise RuntimeError(
            "An error occurred while executing the file download, please check if the link is correct."
        )
    ...
    if isinstance(path, (list, tuple)):
        if "https://" in path[0] or "http://" in path[0]:
            download_dir = (
                ...
                else r"D:\datasets" if not store_dir else store_dir
            )
            download_dir = (
                os.path.join(download_dir, data_name) if data_name else download_dir
            )
