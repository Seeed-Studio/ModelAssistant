    ...
    def load_data_list(self):
        ...
        if dist.is_main_process() and not self._check_exists():
            if not isinstance(backend, LocalBackend):
                raise RuntimeError(f'The dataset on {root} is not integrated, ' f'please manually handle it.')
        ...
        gt_labels = read_label_file(join_path(root, rm_suffix(file_list[1][0])))
...
def read_sn3_pascalvincent_tensor(path: str, strict: bool = True) -> torch.Tensor:
    ...
    ndv = magic % 256
    ty = magic // 256
    assert ndv >= 1 and ndv <= 3
    assert ty >= 8 and ty <= 14
    m = read_sn3_pascalvincent_tensor.typemap[ty]
    s = [get_int(data[4 * (i + 1) : 4 * (i + 2)]) for i in range(ndv)]
    parsed = np.frombuffer(data, dtype=m[1], offset=(4 * (ndv + 1)))
