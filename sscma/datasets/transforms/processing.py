...
class RandomResizedCropCls(BaseTransform):
    ...
    **Modified Keys:**
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = mmcv.imcrop(img, bboxes=np.array([offset_w, offset_h, offset_w + target_w - 1, offset_h + target_h - 1]))
        img = mmcv.imresize(img, tuple(self.scale[::-1]), interpolation=self.interpolation, backend=self.backend)
        results['img'] = img
...
class EfficientNetRandomCrop(RandomResizedCropCls):
    ...
    **Modified Keys:**
    ...
    def __init__(
        self, scale: int, min_covered: float = 0.1, crop_padding: int = 32, interpolation: str = 'bicubic', **kwarg
    ):
...
class RandomErasingCls(BaseTransform):
    ...
    **Modified Keys:**
    ...
    def __init__(
        ...
        assert mode in ['const', 'rand'], 'Please select `mode` from ["const", "rand"].'
        if isinstance(fill_color, Number):
            fill_color = [fill_color] * 3
        assert (
            isinstance(fill_color, Sequence) and len(fill_color) == 3 and all(isinstance(x, Number) for x in fill_color)
...
class EfficientNetCenterCrop(BaseTransform):
    ...
    **Modified Keys:**
    ...
    def __init__(self, crop_size: int, crop_padding: int = 32, interpolation: str = 'bicubic', backend: str = 'cv2'):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img_short = min(h, w)
        crop_size = self.crop_size / (self.crop_size + self.crop_padding) * img_short
        ...
        # crop the image
        img = mmcv.imcrop(
            img, bboxes=np.array([offset_w, offset_h, offset_w + crop_size - 1, offset_h + crop_size - 1])
        )
        # resize image
        img = mmcv.imresize(
            img, (self.crop_size, self.crop_size), interpolation=self.interpolation, backend=self.backend
        )
        results['img'] = img
        results['img_shape'] = img.shape
...
class ResizeEdge(BaseTransform):
    ...
    **Added Keys:**
    ...
    def __init__(self, scale: int, edge: str = 'short', backend: str = 'cv2', interpolation: str = 'bilinear') -> None:
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        img, w_scale, h_scale = mmcv.imresize(
            results['img'], results['scale'], interpolation=self.interpolation, return_scale=True, backend=self.backend
        )
        results['img'] = img
    ...
    def _set_range(self, value, name, center=1, bound=(0, float('inf'))):
        ...
        if isinstance(value, (tuple, list)) and len(value) == 2:
            if not bound[0] <= value[0] <= value[1] <= bound[1]:
                ...
                logger.warning(f'ColorJitter {name} values exceed the bound ' f'{bound}, clipped to the bound.')
        else:
...
class Lighting(BaseTransform):
    ...
    **Modified Keys:**
    ...
    def __init__(self, eigval: Sequence[float], eigvec: Sequence[float], alphastd: float = 0.1, to_rgb: bool = False):
        assert isinstance(eigval, Sequence), f'eigval must be Sequence, got {type(eigval)} instead.'
        assert isinstance(eigvec, Sequence), f'eigvec must be Sequence, got {type(eigvec)} instead.'
        for vec in eigvec:
            assert isinstance(vec, Sequence) and len(vec) == len(
                eigvec[0]
            ), 'eigvec must contains lists with equal length.'
        assert isinstance(alphastd, float), 'alphastd should be of type ' f'float or int, got {type(alphastd)} instead.'
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        img = results['img']
        img_lighting = mmcv.adjust_lighting(img, self.eigval, self.eigvec, alphastd=self.alphastd, to_rgb=self.to_rgb)
...
class AlbumentationsCls(BaseTransform):
    ...
    **Modified Keys:**
    ...
    def __init__(self, transforms: List[Dict], keymap: Optional[Dict] = None):
        ...
        self.aug = albu_Compose([self.albu_builder(t) for t in self.transforms])
    ...
    def albu_builder(self, cfg: Dict):
        ...
        assert isinstance(cfg, dict) and 'type' in cfg, 'each item in ' "transforms must be a dict with keyword 'type'."
        args = cfg.copy()
