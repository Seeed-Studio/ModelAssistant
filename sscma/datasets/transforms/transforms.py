    ...
    def _resize_img(self, results: dict) -> None:
        ...
        if results.get("img", None) is not None:
            ...
            if self.keep_ratio:
                ...
                # a real fix should be done in the simplecv_imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results["img"].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img = F.resize(results["img"], results["scale"], interpolation=InterpolationMode.BILINEAR)
...
class SegRescale(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.scale_factor != 1:
            results["gt_seg_map"] = simplecv_imrescale(
                results["gt_seg_map"],
                self.scale_factor,
                interpolation="nearest",
                backend=self.backend,
...
class MixUp(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        retrieve_img = simplecv_imresize(
            retrieve_img,
            (
