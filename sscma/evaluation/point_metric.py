...
class PointMetric(BaseMetric):
    def __init__(
        self, collect_device: str = "cpu", prefix: Optional[str] = "keypoint"
    ) -> None:
    ...
    def process(self, data_batch: Any, data_samples: Sequence[dict]) -> None:
        ...
        result = np.array(
            [
                i if isinstance(i, np.ndarray) else i.cpu().numpy()
                for i in data_samples[0]["results"]
            ]
        )

        result = (
            result if len(result.shape) == 2 else result[None, :]
        )  # onnx shape(2,), tflite shape(1,2)
