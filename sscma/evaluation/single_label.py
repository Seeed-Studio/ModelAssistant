            ...
            if 'score' in pred_label:
                if self.index_candidates is not None:
                    ...
                    pred_label['score'][..., self.index_candidates] = (
                        pred_label['score'][..., self.index_candidates] + 1.0
                    )
    ...
    **Support** is the number of samples:
        ...
        for data_sample in data_samples:
            ...
            if 'score' in pred_label:
                ...
                assert num_classes is not None, (
                    'The `num_classes` must be specified if `pred_label` has ' 'only `label`.'
                )
                result['pred_label'] = pred_label['label'].cpu()
    ...
    def compute_metrics(self, results: List):
        ...
        if 'pred_score' in results[0]:
            ...
            # If only label in the `pred_label`.
            pred = torch.cat([res['pred_label'] for res in results])
            res = self.calculate(pred, target, average=self.average, num_classes=results[0]['num_classes'])
...
class ConfusionMatrix(BaseMetric):
    ...
    def __init__(
        self, num_classes: Optional[int] = None, collect_device: str = 'cpu', prefix: Optional[str] = None
    ) -> None:
    ...
    def process(self, data_batch, data_samples: Sequence[dict]) -> None:
        for data_sample in data_samples:
            ...
            self.results.append({'pred_label': pred_label, 'gt_label': gt_label})
