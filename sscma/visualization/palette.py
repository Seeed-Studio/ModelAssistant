...
from sscma.utils import simplecv_color_val
...
def get_palette(palette: Union[List[tuple], str, tuple],
    ...
    if isinstance(palette, list):
        ...
        from sscma.datasets import CocoDataset, CocoPanopticDataset
        dataset_palette = CocoDataset.METAINFO['palette']
        if len(dataset_palette) < num_classes:
            dataset_palette = CocoPanopticDataset.METAINFO['palette']
    elif palette == 'citys':
        from sscma.datasets import CityscapesDataset
        dataset_palette = CityscapesDataset.METAINFO['palette']
    elif palette == 'voc':
        from sscma.datasets import VOCDataset
        dataset_palette = VOCDataset.METAINFO['palette']
    elif is_str(palette):
        dataset_palette = [simplecv_color_val(palette)[::-1]] * num_classes
    else:
