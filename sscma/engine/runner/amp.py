...
def autocast(
    ...
    assert digit_version(TORCH_VERSION) >= digit_version('1.5.0'), (
        'The minimum pytorch version requirements of mmengine is 1.5.0, but ' f'got {TORCH_VERSION}'
    )
    ...
    if digit_version('1.5.0') <= digit_version(TORCH_VERSION) < digit_version('1.10.0'):
        ...
        assert device_type == 'cuda' or device_type == 'mlu' or device_type is None, (
            'Pytorch version under 1.10.0 only supports running automatic ' 'mixed training with cuda or mlu'
        )
        ...
        if is_npu_available():
            ...
            if not enabled:
                ...
                raise RuntimeError(
                    'If pytorch versions is between 1.5.0 and 1.10, ' '`autocast` is only available in gpu mode'
                )
        ...
        if device_type == 'cuda':
            ...
            if dtype is None:
                dtype = torch.get_autocast_gpu_dtype()
            with torch.musa.amp.autocast(enabled=enabled, dtype=dtype, cache_enabled=cache_enabled):
                yield
                return
        else:
            ...
            if enabled is False:
                ...
                raise ValueError('User specified autocast device_type must be ' f'cuda or cpu, but got {device_type}')
