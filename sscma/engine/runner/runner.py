...
from sscma.engine.dataloader import CudaDataLoader, MultiEpochsDataLoader
from sscma.registry import (
    DATA_SAMPLERS,
    DATASETS,
    EVALUATOR,
    FUNCTIONS,
...
class Runner:
    ...
    def __init__(
        ...
        if param_scheduler is not None and self.optim_wrapper is None:
            raise ValueError('param_scheduler should be None when optim_wrapper is None, ' f'but got {param_scheduler}')
        ...
        self.logger.info(f'Hooks will be executed in the following ' f'order:\n{self.get_hooks_info()}')
    ...
    def build_visualizer(self, visualizer: Optional[Union[Visualizer, Dict]] = None) -> Visualizer:
        ...
        if visualizer is None:
            visualizer = dict(
                name=self._experiment_name, vis_backends=[dict(type='LocalVisBackend')], save_dir=self._log_dir
            )
    ...
    def build_model(self, model: Union[nn.Module, Dict]) -> nn.Module:
        ...
        if isinstance(model, nn.Module):
            ...
            raise TypeError('model should be a nn.Module object or dict, ' f'but got {model}')
    ...
    def wrap_model(
        ...
        if is_model_wrapper(model):
            if model_wrapper_cfg is not None:
                raise TypeError(
                    'model has been wrapped and "model_wrapper_cfg" should be ' f'None, but got {model_wrapper_cfg}'
                )
        ...
        if not self.distributed:
            ...
            if sync_bn is not None:
                try:
                    ...
                    self.logger.error('cfg.sync_bn should be "torch" or ' f'"mmcv", but got {sync_bn}')
                    raise e
    ...
    def scale_lr(self, optim_wrapper: OptimWrapper, auto_scale_lr: Optional[Dict] = None) -> None:
            ...
            if isinstance(scheduler, _ParamScheduler):
                ...
                param_schedulers.append(
                    PARAM_SCHEDULERS.build(
                        _scheduler, default_args=dict(optimizer=optim_wrapper, epoch_length=len(self.train_dataloader))
                    )
                )
            else:
                raise TypeError('scheduler should be a _ParamScheduler object or dict, ' f'but got {scheduler}')
        return param_schedulers

    def build_param_scheduler(self, scheduler: Union[_ParamScheduler, Dict, List]) -> ParamSchedulerType:
    ...
    def build_dataloader(
        dataloader: Union[DataLoader, Dict], seed: Optional[int] = None, diff_rank_seed: bool = False
    ) -> DataLoader:
        """Build dataloader.
        ...
        if batch_sampler_cfg is None:
            ...
            batch_sampler = DATA_SAMPLERS.build(
                batch_sampler_cfg, default_args=dict(sampler=sampler, batch_size=dataloader_cfg.pop('batch_size'))
            )
        else:
            # fallback to raise error in dataloader
        ...
        async_cfg = dataloader_cfg.pop('async_load', None)
        if async_cfg:
            ...
            if torch.cuda.is_available():
                data_loader = CudaDataLoader(data_loader, 'cuda')
            return data_loader
        else:
    ...
    def build_val_loop(self, loop: Union[BaseLoop, Dict]) -> BaseLoop:
        ...
        if 'type' in loop_cfg:
            loop = LOOPS.build(
                loop_cfg, default_args=dict(runner=self, dataloader=self._val_dataloader, evaluator=self._val_evaluator)
            )
        else:
            loop = ValLoop(
                **loop_cfg, runner=self, dataloader=self._val_dataloader, evaluator=self._val_evaluator
            )  # type: ignore
    ...
    def build_test_loop(self, loop: Union[BaseLoop, Dict]) -> BaseLoop:
        ...
        if 'type' in loop_cfg:
            ...
            loop = TestLoop(
                **loop_cfg, runner=self, dataloader=self._test_dataloader, evaluator=self._test_evaluator
            )  # type: ignore
    ...
    def build_log_processor(self, log_processor: Union[LogProcessor, Dict]) -> LogProcessor:
        ...
        if isinstance(log_processor, LogProcessor):
            ...
            raise TypeError('log processor should be a LogProcessor object or dict, but' f'got {log_processor}')
    ...
    def register_hook(self, hook: Union[Hook, Dict], priority: Optional[Union[str, int, Priority]] = None) -> None:
    ...
    def register_default_hooks(self, hooks: Optional[Dict[str, Union[Hook, Dict]]] = None) -> None:
        ...
        if 'param_schedulers' in checkpoint and resume_param_scheduler:
            ...
            if isinstance(self.param_schedulers, dict):
                ...
                for scheduler, ckpt_scheduler in zip(
                    self.param_schedulers, checkpoint['param_schedulers']  # type: ignore
                ):
                    scheduler.load_state_dict(ckpt_scheduler)
        ...
        if save_optimizer:
            if isinstance(self.optim_wrapper, OptimWrapper):
                checkpoint['optimizer'] = apply_to(
                    self.optim_wrapper.state_dict(), lambda x: hasattr(x, 'cpu'), lambda x: x.cpu()
                )
            else:
                raise TypeError(
    ...
    def _check_scheduler_cfg(self, param_scheduler: Optional[Union[dict, list, _ParamScheduler]]) -> None:
        ...
        if is_seq_of(param_scheduler, dict):
            for _param_scheduler in param_scheduler:
                assert 'type' in _param_scheduler, (
                    'Each parameter scheduler should contain the key type, ' f'but got {_param_scheduler}'
                )
    ...
    def _maybe_compile(self, target: str) -> None:
        ...
        assert isinstance(compile_cfg, dict), f'`compile` should be a dict or bool, got {type(compile_cfg)}'
