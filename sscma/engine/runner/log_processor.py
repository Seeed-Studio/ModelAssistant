...
from sscma.registry import LOG_PROCESSORS
...
class LogProcessor:
    ...
    def get_log_after_iter(self, runner, batch_idx: int, mode: str) -> Tuple[dict, str]:
        ...
        if self.by_epoch:
            ...
            if mode in ['train', 'val']:
                ...
                tag['epoch'] = cur_epoch
                log_str = f'Epoch({mode}){cur_epoch_str}' f'[{cur_iter_str}/{dataloader_len}]  '
            else:
    ...
    def _parse_windows_size(self, runner, batch_idx: int, custom_cfg: Optional[list] = None) -> list:
        ...
        for log_cfg in custom_cfg:
            ...
            if window_size is None or isinstance(window_size, int):
                ...
                raise TypeError('window_size should be int, epoch or global, but got ' f'invalid {window_size}')
        return custom_cfg
