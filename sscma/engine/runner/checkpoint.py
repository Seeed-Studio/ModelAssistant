...
def _get_mmengine_home():
    mmengine_home = os.path.expanduser(
        os.getenv(ENV_MMENGINE_HOME, os.path.join(os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'mmengine'))
    )
...
def load_state_dict(module, state_dict, strict=False, logger=None):
    ...
    def load(module, local_state_dict, prefix=''):
        ...
        module._load_from_state_dict(
            local_state_dict, prefix, local_metadata, True, missing_keys, unexpected_keys, err_msg
        )
...
def _load_checkpoint_to_model(model, checkpoint, strict=False, logger=None, revise_keys=[(r'^module\.', '')]):
...
def save_checkpoint(checkpoint, filename, file_client_args=None, backend_args=None):
    ...
    if filename.startswith('pavi://'):
        if file_client_args is not None or backend_args is not None:
            raise ValueError(
                '"file_client_args" or "backend_args" should be "None" if ' 'filename starts with "pavi://"'
            )
        try:
