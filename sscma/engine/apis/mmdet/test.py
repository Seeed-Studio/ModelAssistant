...
def single_gpu_test_fomo(model, dataloader):
    model.eval()
    results = []
...
def single_gpu_test_mmcls(model, data_loader, audio):
    model.eval()
    results = []
    name = 'audio' if audio else 'img'
            ...
            data.dataset.update(
                {
                    ...
                    'keypoints': torch.tensor(data.dataset['keypoints']).unsqueeze(0),
                }
            )  # pfld
        with torch.no_grad():
