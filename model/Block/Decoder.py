...
class up_sample(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, tag):
        super().__init__()
        if tag == "Conv_block1D":
            self.up_sample = nn.Sequential(nn.ConvTranspose1d(in_channel, out_channel, kernel_size=3, stride=1, padding=1))
        elif tag == "Conv_block2D":
            self.up_sample = nn.Sequential(nn.Conv2d(in_channel, in_channel * 4, kernel_size=3, stride=1, padding=1))
...
class Vae_Decode(nn.Module):
    ...
    def __init__(self, x_size, out_channel, in_channel, tag):
        ...
        self.conv1 = globals()[self.Conv_block](out_channel, out_channel, layer_num=1)
        self.up_sample = up_sample(out_channel, out_channel, tag=self.Conv_block)
        self.conv2 = globals()[self.Conv_block](out_channel, in_channel, layer_num=1)
        self.patch_debed = nn.Conv2d(in_channel, in_channel, kernel_size=3, stride=1, padding=1)
        self.bn = nn.BatchNorm2d(in_channel)
        self.patch_debed2 = nn.Conv2d(in_channel, in_channel, kernel_size=1, stride=1, padding=0)
        self.conv_c_1 = globals()[self.Conv_block](out_channel, out_channel, layer_num=1)
        self.up_sample_c = up_sample(out_channel, out_channel, tag=self.Conv_block)
        self.conv_c_2 = globals()[self.Conv_block](out_channel, in_channel, layer_num=1)
        self.fc_c2 = nn.Linear(middle_dim, self.x_size * self.x_size * out_channel * 4)
        self.patch_debed_c = nn.Conv2d(in_channel, in_channel, kernel_size=1, stride=1, padding=0)
