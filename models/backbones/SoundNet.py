...
class ResBlock1dTF(nn.Module):
    def __init__(self, dim, dilation=1, kernel_size=3):
        ...
        self.block_t = nn.Sequential(
            ...
            nn.Conv1d(dim, dim, kernel_size=kernel_size, stride=1, padding=dilation * (kernel_size // 2), bias=False,
...
class AADownsample(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.conv1d(x, self.filt, stride=self.stride, padding=self.filt_size // 2, groups=x.shape[1])
        return y
...
class Down(nn.Module):
    def __init__(self, channels, d=2, k=3):
        ...
        self.down = nn.Sequential(
            ...
            nn.Conv1d(channels, channels * 2, kernel_size=kk, stride=1, padding=kk // 2, bias=False),
...
class SoundNetRaw(nn.Module):
    def __init__(self, nf=2, clip_length=None, factors=[4, 4, 4], out_channel=48):
        super().__init__()
        base_ = 4
        model = [
            # nn.ReflectionPad1d(3),
            nn.Conv1d(1, nf, kernel_size=7, stride=6, padding=3, bias=False),
            nn.BatchNorm1d(nf),
            nn.LeakyReLU(0.2, True),
        ]
        ...
        for i, f in enumerate(factors):
            ...
            nf *= 2
            if i % 2 == 0:
                model += [ResBlock1dTF(dim=nf, dilation=1, kernel_size=5)]
        ...
        for _, f in enumerate(factors):
            for i in range(1):
                for j in range(3):
                    model += [ResBlock1dTF(dim=nf, dilation=3 ** j, kernel_size=5)]
            model += [Down(channels=nf, d=f, k=f * 2 + 1)]
            nf *= 2
