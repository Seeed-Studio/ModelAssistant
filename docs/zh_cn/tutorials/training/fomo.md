...
[ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) 默认使用 [COCO_MASK 数据集](../datasets.md#ModelAssistant)训练 FOMO 模型，请参照以下步骤完成数据集的准备。

1. 参考[互联网数据集 - ModelAssistant - COCO_MASK 数据集](../datasets.md#ModelAssistant)下载数据集并完成数据集的解压。
...
对于 FOMO 模型示例，我们使用 `fomo_mobnetv2_0.35_x8_abl_coco.py` 作为配置文件，它位于 [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) 主目录路径 `configs/fomo` 下的文件夹中，并额外继承了 `default_runtime_det.py` 配置文件。
...
然后，在 [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant)  项目根目录，我们执行如下的指令，训练一个 FOMO 口罩识别模型。
