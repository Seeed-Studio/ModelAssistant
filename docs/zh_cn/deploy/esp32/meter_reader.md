...
本教程将基于 [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) 示范使用 ESP32 实现表计读数的开发流程。
...
您需要将 `<TFLITE_MODEL_PATH>` 替换为在[导出模型](#%E5%AF%BC%E5%87%BA%E6%A8%A1%E5%9E%8B)步骤取得的 TFLite 模型的路径，转换生成的 C 文件将默认输出到 `ModelAssistant/example/esp32` 目录中的 `components/modules/model` 目录。
