...
[ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) uses [COCO_MASK Datasets](../datasets.md#ModelAssistant) by default to train the FOMO model, please refer to the following steps to complete the preparation of datasets.

1. Please refer to \[Internet Datasets - [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) - COCO_MASK Dataset\](../datasets.md#ModelAssistant) to download and unpack the dataset.
...
For the FOMO model example, we use `fomo_mobnetv2_0.35_x8_abl_coco.py` as the configuration file, which is located in the folder under the [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) root directory `configs/fomo` and its additionally inherits the `default_runtime_det.py` configuration file.
...
Training the model requires using our previously configured [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) working environment, if you follow our [Installation](../../introduction/installation.md) guide using Conda to install [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) in a virtual environment named `sscma`, please first make sure that you are currently in the virtual environment.

Then, in the [ModelAssistant](https://github.com/Seeed-Studio/ModelAssistant) project root directory, we execute the following command to train a FOMO mask detection model.
